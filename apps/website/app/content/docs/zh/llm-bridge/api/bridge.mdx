...
<Tabs items={['BridgeOptions', 'BridgeConfig']}>
  ...
  <Tab value="BridgeConfig">
    ...
    interface BridgeConfig {
      apiKey: string            // 出站提供商的 API 密钥（必填）
    ...
    | 属性 | 类型 | 默认值 | 描述 |
    |------|------|--------|------|
    | `apiKey` | `string` | - | 用于身份验证的 API 密钥（必填） |
    ...
    | `authHeaderPrefix` | `string?` | `'Bearer'` | 认证请求头值的前缀。设置为空字符串 `''` 可以不添加前缀直接使用 API 密钥 |
  </Tab>
</Tabs>
...
列出出站提供商的可用模型。向提供商的 models 端点发送 GET 请求。
