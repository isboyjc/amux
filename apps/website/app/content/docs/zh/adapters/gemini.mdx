...
```bash tab="pnpm"
pnpm add @llm-bridge/core @llm-bridge/adapter-gemini
```

```bash tab="npm"
npm install @llm-bridge/core @llm-bridge/adapter-gemini
```
...
<Callout type="warn">
Gemini API 使用不同的请求格式（`contents` 而不是 `messages`）。LLM Bridge 会自动处理格式转换。
</Callout>
...
<Cards>
  ...
  <Card title="适配器概览" href="/docs/zh/adapters">
    查看所有可用的适配器
  </Card>
</Cards>
