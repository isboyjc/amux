...
<Callout type="info">
  **关于工具调用：** 工具调用使用 Message 对象上的 `toolCalls` 字段（OpenAI
  风格）表示，而不是 ContentPart 类型。详见[工具调用流程](#工具调用流程)部分。
</Callout>
...
<Callout type="info">
  **统一的工具调用格式：** IR 使用 OpenAI 风格的工具调用作为统一格式。当从
  Anthropic 格式转换时，适配器会自动将 `tool_use` 内容块转换为 `toolCalls`，将
  `tool_result` 内容块转换为 tool 角色消息。
</Callout>
