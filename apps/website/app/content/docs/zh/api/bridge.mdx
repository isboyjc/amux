...
<Tabs items={['BridgeOptions', 'BridgeConfig']}>
  ...
  <Tab value="BridgeConfig">
    ...
    interface BridgeConfig {
      apiKey: string            // 出站提供商的 API 密钥（必需）
      ...
      authHeaderPrefix?: string // 认证请求头前缀（默认：'Bearer'，设为空字符串表示无前缀）
    }
    ```

    | 属性 | 类型 | 默认值 | 描述 |
    |------|------|--------|------|
    | `apiKey` | `string` | - | **必需** 用于身份验证的 API 密钥 |
    ...
    | `authHeaderPrefix` | `string?` | `'Bearer'` | 认证请求头值的前缀（设为 `''` 表示无前缀） |
  </Tab>
</Tabs>
...
// 添加 [DONE] 标记表示流结束（由用户的 HTTP 层处理）
res.write('data: [DONE]\n\n')
res.end()
```

<Callout type="info">
**关于 `[DONE]` 标记**：Bridge 会过滤掉提供商响应中的 `[DONE]` 标记。您需要在 HTTP 层添加 `data: [DONE]\n\n` 标记来向客户端发送流结束信号。
</Callout>

<Callout type="info">
**关于 stream_options**：启用流式传输时，Bridge 会自动设置 `stream_options: { include_usage: true }`，以在最终的流事件中包含使用统计信息。
</Callout>
...
<Callout type="warn">
注意：并非所有提供商都支持列出模型。例如，MiniMax 没有提供模型列表端点。
</Callout>
