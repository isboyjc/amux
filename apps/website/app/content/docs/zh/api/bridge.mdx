...
<Tabs items={['BridgeOptions', 'BridgeConfig']}>
  <Tab value="BridgeOptions">
    ...
    interface BridgeOptions {
      ...
      hooks?: BridgeHooks   // 生命周期钩子（IR 级可观测）
    ...
    | `hooks` | `BridgeHooks?` | IR 级请求/响应/流/错误生命周期钩子 |
...
<Callout type="info">
  **v0.1.0 新特性**: Bridge 现在支持生命周期钩子，用于在 IR (中间表示层)
  进行监控、日志记录和自定义处理。
</Callout>
...
<Callout type="warn">
  请求必须使用**入站适配器的格式**。响应也将使用入站适配器的格式，即使内部调用的是不同的提供商
  API。
</Callout>
...
<Callout type="info">
  当你希望拿到提供商格式 SSE（`event` + `data`）并直接写回 HTTP 响应时，使用
  `chatStream()`；当你需要统一 IR
  事件（`event.type`、`event.content.delta`）时，使用 `chatStreamRaw()`。
</Callout>
...
<Callout type="info">
  `chatStreamRaw()` 会在内部强制 `stream = true`，并过滤重复 `end` 事件；Bridge
  级别的 `chatStream()` 会过滤协议标记 `[DONE]`，避免把它作为业务数据继续输出。
</Callout>
...
<Callout type="warn">
  `listModels()` 依赖适配器的 `endpoint.modelsPath` 或显式配置
  `config.modelsPath`。部分提供商（例如 MiniMax
  适配器默认配置）不提供模型列表端点。
</Callout>
