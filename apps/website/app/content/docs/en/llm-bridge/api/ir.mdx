...
<Tabs items={['Request', 'Response', 'Message', 'Tool', 'Stream', 'Error']}>
  ...
  <Tab value="Response">
    ...
    interface LLMResponseIR {
      ...
      created?: number                 // Response creation timestamp
...
The `Usage` interface provides detailed token usage information:
...
interface Usage {
  ...
  details?: {
    reasoningTokens?: number      // Reasoning/thinking tokens (DeepSeek, Qwen)
    cachedTokens?: number        // Cached prompt tokens (DeepSeek, Anthropic)
    cacheCreationTokens?: number // Cache creation tokens (Anthropic)
    cacheReadTokens?: number    // Cache read tokens (Anthropic)
  }
}
```

### Token Details by Provider

| Provider | reasoningTokens | cachedTokens | cacheCreationTokens | cacheReadTokens |
|----------|-----------------|--------------|---------------------|-----------------|
| OpenAI | - | - | - | - |
| Anthropic | - | ✅ | ✅ | ✅ |
| DeepSeek | ✅ | ✅ | - | - |
...
<Callout type="info">
The `details` field is optional and only included when the provider provides these additional token counts. This allows for more granular cost tracking and caching optimization.
</Callout>
