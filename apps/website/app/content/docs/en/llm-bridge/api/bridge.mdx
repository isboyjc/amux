...
<Tabs items={['BridgeOptions', 'BridgeConfig']}>
  ...
  <Tab value="BridgeConfig">
    ...
    | `authHeaderPrefix` | `string?` | `'Bearer'` | Prefix for the authentication header value. Set to empty string `''` if the provider expects the API key without a prefix |
  </Tab>
</Tabs>
...
Lists available models from the outbound provider. Sends a GET request to the provider's models endpoint.
...
console.log(models)
// Returns provider-specific model list format
```
