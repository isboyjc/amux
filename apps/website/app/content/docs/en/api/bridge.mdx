...
<Tabs items={['BridgeOptions', 'BridgeConfig']}>
  ...
  <Tab value="BridgeConfig">
    ...
    interface BridgeConfig {
      apiKey: string            // API key for the outbound provider (required)
      ...
      authHeaderPrefix?: string // Authentication header prefix (default: 'Bearer', set to '' for no prefix)
    }
    ```
    ...
    | `authHeaderPrefix` | `string?` | `'Bearer'` | Prefix for the authentication header value (set to `''` for no prefix) |
  </Tab>
</Tabs>
...
// Add [DONE] marker to signal stream end (handled by user's HTTP layer)
res.write('data: [DONE]\n\n')
res.end()
```

<Callout type="info">
**Note on `[DONE]` marker**: The Bridge filters out `[DONE]` markers from the provider response. You need to add the `data: [DONE]\n\n` marker in your HTTP layer to signal the stream end to the client.
</Callout>

<Callout type="info">
**Note on stream_options**: When streaming is enabled, Bridge automatically sets `stream_options: { include_usage: true }` to include usage statistics in the final stream event.
</Callout>
...
Lists available models from the target provider.
...
<Callout type="warn">
Note: Not all providers support listing models. For example, MiniMax does not provide a models list endpoint.
</Callout>
