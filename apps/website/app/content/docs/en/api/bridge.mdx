...
```typescript
function createBridge(options: BridgeOptions): LLMBridge
```
...
<Tabs items={['BridgeOptions', 'BridgeConfig']}>
  <Tab value="BridgeOptions">
    ...
    interface BridgeOptions {
      ...
      hooks?: BridgeHooks   // Lifecycle hooks (IR-level observability)
    ...
    | `hooks` | `BridgeHooks?` | Lifecycle hooks for IR-level request/response/stream/error events |
...
<Callout type="info">
  **New in v0.1.0**: Bridge now supports lifecycle hooks for monitoring,
  logging, and custom processing at the IR (Intermediate Representation) layer.
</Callout>
...
<Callout type="warn">
  The request must be in the **inbound adapter's format**. The response will
  also be in the inbound adapter's format, even though a different provider API
...
<Callout type="info">
  Use `chatStream()` when you want provider-formatted SSE events (`event` +
  `data`) that can be written directly to HTTP responses. Use `chatStreamRaw()`
  if you need normalized IR events (`event.type`, `event.content.delta`, etc.).
</Callout>
...
<Callout type="info">
  `chatStreamRaw()` enforces `stream = true` internally, filters duplicate `end`
  events, and Bridge-level `chatStream()` filters protocol marker `[DONE]` from
  final emitted events.
</Callout>

### listModels

Lists available models from the outbound provider.
...
<Callout type="warn">
  `listModels()` requires either adapter `endpoint.modelsPath` or explicit
  `config.modelsPath`. Some providers (for example MiniMax adapter default
  config) don't define a models endpoint by default.
</Callout>
...
<Callout type="info">
  Use this method to verify that your adapter combination supports all the
  features you need before making requests.
</Callout>
