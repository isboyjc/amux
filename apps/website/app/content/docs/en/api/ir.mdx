...
<Callout type="info">
  **Note on Tool Calls:** Tool calls are represented using the `toolCalls` field
  on the Message object (OpenAI-style), not as ContentPart types. See the [Tool
  Calling Flow](#tool-calling-flow) section for details.
</Callout>
...
<Callout type="info">
  **Unified Tool Call Format:** IR uses OpenAI-style tool calling as the unified
  format. When converting from Anthropic format, the adapter automatically
  converts `tool_use` content blocks to `toolCalls` and `tool_result` content
  blocks to tool role messages.
</Callout>
