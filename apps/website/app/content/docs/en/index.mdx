---
title: Introduction
description: Welcome to Amux - a unified infrastructure for converting between different LLM provider APIs.
---

## What is Amux?

Amux is a bidirectional API adapter that allows you to:

- 🔄 **Convert between any LLM provider formats** - OpenAI, Anthropic, DeepSeek, Moonshot, Zhipu, Qwen, Gemini
...
<Callout type="info">
Amux enables **true bidirectional conversion** - accept requests in any format, call any provider API, and return responses in any format.
</Callout>
...
<Tabs items={['Bidirectional', 'Type Safety', 'Extensibility', 'Performance']}>
  <Tab value="Bidirectional">
    ...
    Unlike other solutions that only provide a unified interface, Amux allows you to:
  ...
  <Tab value="Type Safety">
    ...
    ```typescript
    import type { LLMRequestIR, LLMResponseIR } from '@amux/llm-bridge'
...
<Callout type="info">
Amux is perfect for applications that need to support multiple LLM providers or migrate between providers without changing application code.
</Callout>
