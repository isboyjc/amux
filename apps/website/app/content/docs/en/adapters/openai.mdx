...
```bash tab="yarn"
yarn add @amux.ai/llm-bridge @amux.ai/adapter-openai
```
...
<Callout type="info">
  **Recommendation**: Use `openaiAdapter` for production applications requiring
  stability. Use `openaiResponsesAdapter` when you need the latest features like
  built-in web search, reasoning models (o3, o4-mini), or stateful multi-turn
  conversations.
</Callout>

<Callout type="warn">
  **Important**: The standard `openaiAdapter` does NOT support reasoning models
  (o3, o4-mini). Reasoning capabilities are ONLY available through
...
<Callout type="info">
  Reasoning models (o3, o4-mini) are only available through the Responses API
  (`openaiResponsesAdapter`). They support reasoning effort control via the
...
<Callout type="warn">
  **GPT-5 Series Special Requirements:** - Temperature parameter must be set to
  1 - Use `max_completion_tokens` instead of `max_tokens` - Do not pass `top_p`
  parameter
</Callout>

<Callout type="info">
  Model list may be updated. Check the [OpenAI
...
When using the Bridge directly, the request format remains the same (IR format):
...
// Request format remains the same (IR format)!
const response = await bridge.chat({
  model: 'gpt-4',
...
<Callout type="warn">
**Important for Application Developers**: If you're building an application (web app, CLI, etc.) that sends HTTP requests directly to an endpoint using `openaiResponsesAdapter`, you MUST send requests in the Responses API format, not Chat Completions format.

```typescript
// ❌ Wrong: Sending Chat Completions format to Responses API endpoint
fetch('/v1/responses', {
  body: JSON.stringify({
    model: 'gpt-4',
...
// ✅ Correct: Sending Responses API format
fetch('/v1/responses', {
  body: JSON.stringify({
    model: 'gpt-4',
    input: [
...
Each adapter has its own standard format. Your application layer should adapt to the adapter's format, not the other way around.
