...
<Tabs items={['Code Generation', 'Function Calling', 'Streaming']}>
...
## Reasoner Model Boundaries

When using `deepseek-reasoner`, note the following adapter-side guardrails:

- System messages are skipped for reasoner models.
- Input `reasoning_content` is not passed through for reasoner inputs.
- `generation.maxTokens` is clamped to `1..8192` when building outbound requests.
  ...
  messages: [
    {
      role: 'system',
      content:
        'You are a professional programmer. Generate clean, efficient, well-commented code.',
    },
    {
      role: 'user',
