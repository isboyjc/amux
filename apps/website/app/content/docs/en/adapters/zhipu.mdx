...
<Callout type="info">
  Zhipu AI models are continuously updated. Please refer to the official
  documentation for the latest model list.
</Callout>
...
| Vision           | ⚠️        | Capability declared; current outbound adapter path mainly concatenates text parts |
| System Prompt    | ✅        | Fully supported                                                                   |
| JSON Mode        | ✅        | Fully supported                                                                   |
| Web Search       | ⚠️        | Capability declared; no dedicated request-field mapping in current adapter        |

<Callout type="warn">
  Current adapter boundary notes:

- `tool_choice = 'required'` is filtered out when building outbound Zhipu
  requests.
- Multimodal content parts are currently flattened to text in outbound
...
const complexTask = await bridge.chat({
  ...
  messages: [
    {
      role: 'user',
      content: 'Please analyze the performance issues in this code...',
    },
  ],
})
```
  ...
  messages: [
    {
      role: 'system',
      content:
        'You are a professional technical documentation writer. Please answer questions in concise, professional language.',
    },
    {
      role: 'user',
