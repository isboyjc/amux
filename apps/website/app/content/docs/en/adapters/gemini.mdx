...
<Callout type="warn">
  Gemini API uses a different request format (`contents` instead of `messages`).
  Amux handles format conversion automatically.
</Callout>
...
<Callout type="info">
  Gemini's key feature is ultra-long context (up to 2M tokens), far exceeding
  other models.
</Callout>
...
<Callout type="warn">
  Gemini's function calling format differs from OpenAI and may require
...
<Callout type="warn">
  Although Gemini request types include `safetySettings`, the current adapter
  outbound request builder does not map this field from IR-level request
  examples directly.
</Callout>
