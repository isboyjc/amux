...
<Callout type="info">
  Amux normalizes tool calling formats across all providers, allowing you to use
  the same code with OpenAI, Anthropic, DeepSeek, and others.
</Callout>
...
const answer = await chatWithTools(
  'What is the weather in Tokyo? Also calculate 15 * 24.'
)
console.log(answer)
```
...
<Callout type="warn">
  Some providers may have limited tool choice options. Check adapter
