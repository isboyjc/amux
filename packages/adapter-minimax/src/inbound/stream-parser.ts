...
import type { MinimaxStreamChunk } from '../types'

/**
 * Map MiniMax finish reason to IR finish reason
 */
...
/**
 * Parse MiniMax stream chunk to IR stream event
 */
export function parseStream(
  chunk: unknown
): LLMStreamEvent | LLMStreamEvent[] | null {
  const data = chunk as MinimaxStreamChunk
  ...
  if (delta.role && !delta.content && !delta.tool_calls && !delta.reasoning_details) {
    return {
      type: 'start',
      id: data.id,
      model: data.model,
