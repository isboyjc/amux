...
describe('Gemini Adapter', () => {
  ...
  describe('Outbound - buildRequest', () => {
    it('should build Gemini request from IR', () => {
      ...
      const request = geminiAdapter.outbound.buildRequest(ir) as {
    ...
    it('should build request with vision content from IR', () => {
      ...
      const request = geminiAdapter.outbound.buildRequest(ir) as {
        contents: Array<{ parts: Array<{ text?: string; fileData?: { fileUri: string } }> }>
      }

      expect(request.contents).toHaveLength(1)
      expect(request.contents[0]?.parts).toHaveLength(2)
      expect(request.contents[0]?.parts[0]?.text).toBe('What is in this image?')
      expect(request.contents[0]?.parts[1]?.fileData?.fileUri).toBe('https://example.com/image.jpg')
    })
  })
