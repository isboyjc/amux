...
import type { KimiRequest, KimiMessage, KimiTool } from '../types'

/**
 * Parse Kimi request to IR
 */
export function parseRequest(request: unknown): LLMRequestIR {
  const req = request as KimiRequest
...
function parseMessage(msg: KimiMessage): Message {
  return {
    role: msg.role,
    content: msg.content ?? '',
    name: msg.name,
...
function parseTool(tool: KimiTool): Tool {
  return {
    type: 'function',
    function: {
